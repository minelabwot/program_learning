...
public final class GenericUserPreferenceArray implements PreferenceArray {
  ...
  //最终将userID合并，并单独罗列出itemIDs，prefValues
  //传进 Collection<GenericPreference>
  //GenericPreference->(userID,itemID,preferenceValue)
  public GenericUserPreferenceArray(List<? extends Preference> prefs) {
	  //初始化数组ids，values并将id置为Long类型的最小值
    this(prefs.size());
    int size = prefs.size();
  ...
  //type=item=1，也就是对itemID进行比较
  //对ids和values进行排序，从小到大进行排序
