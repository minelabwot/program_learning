...
////////////////////////////////////////////////////////////////////////////////////////////////////
    function getContent($url,$channel,$Gnum,$p_area)
	{
	    ...
	    //获取节目图标
	    $p_img=$html->find('div[id="TvChannelDIV"]',0);
	    // print_r("\np_img=".$p_img."\n");
	    $p_image=$p_img->find('img',0)->src;
	    // print("\np_image=".$p_image."\n");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////		   
		...
		for ($i=0; $i < count($program); $i++)
		{
			...
			if(isset($program[$i]['content']))
			{
				...
				//去除爬取中主持人和主演的乱码字符，除了字母、·、空格、逗号、句号、、、、省略号、中文之外的任意字符	  
				preg_replace("[^a-z\A-Z\·\s\"\，\。\、\…\u4e00-\u9fa5]", "", $program[$i]['content']);
			...
			//去除爬取中主持人和主演的乱码字符，除了字母、·、空格、中文之外的任意字符
			preg_replace("[^a-z\A-Z\·\s\u4e00-\u9fa5]", "“", $program[$i]['host']);
			...
			if(isset($program[$i]['p_image']))	   
				$program[$i]['p_image']=iconv('UTF-8','GB2312//IGNORE', $program[$i]['p_image']);
			...
			$image1=iconv('UTF-8','GB2312//IGNORE','节目图标');
			$pimage1=iconv('UTF-8','GB2312//IGNORE','频道图标');
			$duration1=iconv('UTF-8','GB2312//IGNORE','播出时长');
			$abs1=iconv('UTF-8','GB2312//IGNORE','其他');

			$query="INSERT INTO `program_new`(`$Gnum1`,`$channel1`,`$t1`,`$date1`,`$day1`,`$n1`,`$c1`,`$type1`,`$host1`,`$area1`,`$image1`,`$pimage1`,`$duration1`,`$abs1`) VALUES ($Gnum,$channel,$t,$date,$day,$n,$c,$type,$host,$area,$image,$pimage,$duration,$abs)";
			mysql_query($query);
			print mysql_error();
		}
		print "database ok!\n";
